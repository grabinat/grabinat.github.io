(function () {
    if (typeof Mark !== "function") {
        ...
        Mark.context = document.getElementsByTagName('body')[0];
        var el = document.createElement("style");
        el.innerText = "mark.current{background:orange}";
        Mark.context.appendChild(el);
        Mark.results;
        Mark.currentClass = "current",
            Mark.currentIndex = 0;

        Mark.unmarkAll = function unmarkAll() {
            var inst = new Mark(Mark.context);
            inst.unmark();
        }

        Mark.search = function search(text, wholeWordSearch, caseSensitive) {
            var el;
            var inst = new Mark(Mark.context);
            ...
            var options = {
                ...
                "accuracy": wholeWordSearch,
                "done": function (counter) {
                    Mark.currentIndex = 0;

                    //find hidden elements and mark them as mark_hidden
                    var hidden = $('mark').filter(function () { return !(this.offsetWidth > 0 && this.offsetHeight > 0) }).each(function () { this.className += " mark_hidden" });

                    Mark.results = counter - hidden.length;

                    var opt = {
                        "className": "mark_hidden",
                        "done": function () {
                            Mark.marked = document.getElementsByTagName('mark');
                            Mark.jumpTo()
                        }
                    }
                    inst.unmark(opt);
            ...
            inst.unmark({
                    ...
                    inst.mark(text, options);
                }
            });
        }

        Mark.jumpTo = function jumpTo() {
            var index = 1;
            if (Mark.results == 0)
                index = 0;
            var status = {
                ...
                Error: "",
                Message: {
                    Current: Mark.currentIndex + index,
                    SearchResult: Mark.results
                }
            }
            window.external.notify(JSON.stringify(status));

            if (Mark.results > 0) {
                var position;
                var current;

                if (Mark.marked.length > 0)
                    current = Mark.marked[Mark.currentIndex];

                //remove current class
                var currentclass = document.getElementsByClassName(Mark.currentClass);
                if (currentclass.length > 0) {
                    currentclass[0].className = "";
                }
                if (current) {

                    current.className = Mark.currentClass;

                    var scroll = Mark.getScrollParent(current);
        ...
        Mark.getScrollParent = function getScrollParent(node) {
            if (node === null) {
                return null;
            }
        ...
        Mark.next = function next() {
            if (Mark.results > 0) {
                Mark.currentIndex++;
                if (Mark.currentIndex >= Mark.results)
                    Mark.currentIndex = 0;
        ...
        Mark.prev = function prev() {
            if (Mark.results > 0) {
                Mark.currentIndex--;
                if (Mark.currentIndex < 0)
                    Mark.currentIndex = Mark.results - 1;
